...
void			Control_Signal(unsigned int opcode, unsigned int inst);
...
void			Set_ID_EX(unsigned int pc_add_4, unsigned int inst, unsigned int extended);
...
int main(void)
{
	...
	while (pc < 64 | mem_wb.is_activated)
	{
		...
		Control_Signal(inst_31_26, inst);
		...
		Set_ID_EX(pc_add_4, inst, inst_ext_32);
...
void	Control_Signal(unsigned int opcode, unsigned int inst)
{
	...
	control.RegDst		= (inst == 0 ? 0 : R_format);
	control.Jump		= (inst == 0 ? 0 : Jump);
	control.Branch		= (inst == 0 ? 0 : BEQ);
	control.MemRead		= (inst == 0 ? 0 : Lw);
	control.MemtoReg	= (inst == 0 ? 0 : Lw);
	control.ALUOp		= (inst == 0 ? 0 : (R_format << 1) + BEQ);
	control.MemWrite	= (inst == 0 ? 0 : Sw);
	control.ALUSrc		= (inst == 0 ? 0 : Lw || Sw);
	control.RegWrite	= (inst == 0 ? 0 : R_format || Lw);
}
...
void	Set_ID_EX(unsigned int pc_add_4, unsigned inst, unsigned int extended)
{
	if (pc_add_4 < 64)
		id_ex.is_activated = 1;
	else
		id_ex.is_activated = 0;
	...
	id_ex.pc_add_4 = pc_add_4;
	id_ex.RegRs = (inst & 0x03e00000) >> 21;
	id_ex.RegRt = (inst & 0x001f0000) >> 16;
	id_ex.RegRd = (inst & 0x0000f800) >> 11;
	id_ex.extended_immediate = extended;
	id_ex.reg_read = reg_read;

	id_ex.inst_31_26_for_print = inst >> 26;
}
...
void	print_assignment(void)
{
	...
	printf("EX - RegDst   : %d, ALUOp    : %d%d, ALUSrc   : %d, Jump : %d\n", id_ex.RegDst, id_ex.ALUOp / 2, id_ex.ALUOp % 2, id_ex.ALUSrc, id_ex.Jump);
	printf(" === EX/MEM === \n");
...
void	print_pipeline_regi_state(unsigned int pc)
{
	printf("             | ==== ID/EX ==== | ==== EX/MEM ==== | ==== MEM/WB ==== \n");
	printf("-------------+-----------------+------------------+------------------┐ \n");
	printf("   Activated | %15d | %16d | %16d |\n", id_ex.is_activated, ex_mem.is_activated, mem_wb.is_activated);
	printf("-------------+-----------------+------------------+------------------┐ \n");
	printf("      RegDst | %15d | %16s | %16s |\n", id_ex.RegDst, None, None);
	printf("       ALUOp | %15d | %16s | %16s |\n", id_ex.ALUOp, None, None);
	printf("      ALUSrc | %15d | %16s | %16s |\n", id_ex.ALUSrc, None, None);
	printf("        Jump | %15d | %16s | %16s |\n", id_ex.Jump, None, None);
	printf("      Branch | %15d | %16d | %16s |\n", id_ex.Branch, ex_mem.Branch, None);
	printf("     MemRead | %15d | %16d | %16s |\n", id_ex.MemRead, ex_mem.MemRead, None);
	printf("    MemWrite | %15d | %16d | %16s |\n", id_ex.MemWrite, ex_mem.MemWrite, None);
	printf("    RegWrite | %15d | %16d | %16d |\n", id_ex.RegWrite, ex_mem.RegWrite, mem_wb.RegWrite);
	printf("    MemtoReg | %15d | %16d | %16d |\n", id_ex.MemtoReg, ex_mem.MemtoReg, mem_wb.MemtoReg);
	printf("-------------+-----------------+------------------+------------------┤ \n");
	printf("      Opcode | %15d | %16d | %16d |\n", id_ex.inst_31_26_for_print, ex_mem.inst_31_26_for_print, mem_wb.inst_31_26_for_print);
	printf("  Reg_read_1 | %15d | %16s | %16s |\n", id_ex.reg_read.Read_data_1 , None, None);
	printf("  Reg_read_2 | %15d | %16d | %16s |\n", id_ex.reg_read.Read_data_2 , ex_mem.Write_data, None);
	printf("       RegRs | %15d | %16s | %16s |\n", id_ex.RegRs, None, None);
	printf("       RegRt | %15d | %16s | %16s |\n", id_ex.RegRt, None, None);
	printf("       RegRd | %15d | %16d | %16d |\n", id_ex.RegRd, ex_mem.RegRd , mem_wb.RegRd);
	printf("  ALU_result | %15s | %16d | %16d |\n", None, ex_mem.alu.ALU_result, mem_wb.ALU_result);
	printf("  MEM_result | %15s | %16s | %16d |\n", None, None, mem_wb.MEM_result);
	printf(" Branch_addr | %15s | %16d | %16s |\n", None, ex_mem.branch_addr, None);
	printf("-------------+-----------------+------------------+------------------┘ \n");
}
